...
> **Nota sobre `overwriteSchema`**: el notebook de ingesta escribe con `.option('overwriteSchema', 'true')` en todos los writes Delta. Esto es necesario cuando las tablas ya existen con un schema previo (p.ej. columnas `_c0` generadas por una ejecución anterior con `inferSchema`), ya que Delta rechaza por defecto cualquier cambio de schema sin esta opción. Con `mode('overwrite')` + `overwriteSchema=True` las tablas se recrean completamente con el schema correcto.
